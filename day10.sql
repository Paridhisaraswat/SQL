...
-- get all the student detail whether they have enrolled in any cousre or not 
select s.student_id , s.student_name , s.major , e.enrollment_id , e.course_name  from students as s
left join 
enrollments as e
on s.student_id = e.student_id ;

-- get those student where they have not enrolled in any course 
select s.student_id , s.student_name , s.major , e.enrollment_id , e.course_name  from students as s
left join 
enrollments as e
on s.student_id = e.student_id 
...
-- get every student name and the total num of course the student has enrolled 
...
-- get the students who are enrolled in more than 2 courses 
select s.student_id , s.student_name , count(e.enrollment_id) from students as s
left join 
enrollments as e
on s.student_id = e.student_id
...
-- find the number of student in each major 
...
-- write the sql query to group students by the length of their name 
-- show the count of students in each group 

select char_length(student_name) ,count(student_id) from students
group by char_length(student_name);

-- find out in each grp how many student have enrolled in course   
-- num of courses enrolled for each grp name 
select char_length(s.student_name),count(e.enrollment_id) from students as s
join enrollments as e on s.student_id = e.student_id 
group by char_length(student_name);

-- show the particular grp which have the highest number of enrollments 
select char_length(s.student_name) , count(e.enrollment_id) from students as s 
join enrollments as e on s.student_id = e.student_id 
group by char_length(student_name)
order by count(e.enrollment_id) desc limit 1; 
-- LIMIT : show the num of rows after order by 
