...
sum(population) over(),sum(population) over(partition by continent),
avg(population)over(),avg(population)over(partition by continent) from country ;
...
-- OVER ==> to order a set of rows in ascending or desc order 
...
-- WINDOWS FUNCTION 
-- row_number() ==> define a unique value to a row 
...
-- if ranking are shared the next ranking is skiped 
select * , rank() over (order by city) from employees ;


-- DENSE_RANK()
-- if ranking is hared the next rank is not skiped
select * , rank() over (order by city) , dense_rank() over(order by city)from employees ;
 
select * , dense_rank() over(partition by city) from employees ;
